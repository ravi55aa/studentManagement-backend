...
implements IBatchRepository{
    ...
    async getAllBatches(query:FilterQuery<Partial<IBatches>>):Promise<IBatches[]>{
        return await batchModel.find({tenantId:query.tenantId}).populate("batchCounselor", "_id firstName").lean<IBatches[]>();
    }
    ...
    public async findByTeacherId(
        ...
        return batchModel
            .findOne({ batchCounselor: teacherId })
            .lean<IBatches>();
        }
    ...
    public async assignTeacher(
        ...
        return batchModel.findByIdAndUpdate(
            batchId,
            { $set: { batchCounselor: teacherId } },
            { new: true }
        ).lean<IBatches>();
        }
