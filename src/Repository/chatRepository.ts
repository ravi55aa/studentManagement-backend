...
export class ChatRoomRepository extends BaseRepository<IChatRoom> implements IChatRoomRepository {
  ...
  //  Create Chat Room
  async createChatRoom(data: Partial<IChatRoom>): Promise<IChatRoom | null> {
    try {
  ...
  async findDirectChat(user1: string, user2: string): Promise<IChatRoom | null> {
    try {
  ...
  // Update last message
  async updateLastMessage(chatRoomId: string, message: string): Promise<IChatRoom | null> {
    try {
  ...
  async addParticipant(chatRoomId: string, userId: string): Promise<IChatRoom | null> {
    try {
      return await this.model.findByIdAndUpdate(
        chatRoomId,
        {
  ...
  // Remove participant
  async removeParticipant(chatRoomId: string, userId: string): Promise<IChatRoom | null> {
    try {
      return await this.model.findByIdAndUpdate(
        chatRoomId,
        {
 ...
 */
@injectable()
export class ChatMessageRepository extends BaseRepository<IMessage> implements IMessageRepository {
  constructor() {
    super(messageModel);
  }
  ...
  async getMessages(chatRoomId: string, limit?: number): Promise<IMessage[]> {
    try {
      return await this.model
        .find({ chatRoomId })
  ...
  async markAllAsRead(chatRoomId: string, userId: string): Promise<boolean> {
    try {
  ...
  async getLastMessage(chatRoomId: string): Promise<IMessage | null> {
    try {
      return await this.model.findOne({ chatRoomId }).sort({ createdAt: -1 }).lean<IMessage>();
    } catch (error) {
