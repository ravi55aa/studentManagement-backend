...
export const schoolMetaDataValidateSchema = z
  .object({
...
export const schoolAddressValidateSchema = z.object({
...
export const schoolSubjectSchema = z
  .object({
    name: z.string().min(3, 'Subject name must be at least 3 characters').trim(),
    ...
    type: z.enum(['theory', 'practical', 'both'], { message: 'Subject type is required' }),
    ...
    level: z
      .enum(['primary', 'secondary', 'higher-secondary', 'degree'], { message: 'Invalid level' })
      .optional(),
    ...
    referenceBooks: z.array(z.string().url('Invalid file URL')).optional(),
