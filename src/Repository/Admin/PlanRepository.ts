...
export class PlanRepository extends BaseRepository<IPlan> implements IPlanRepository {
  ...
  async findByName(name: string): Promise<IPlan | null> {
    try {
      return await this.model.findOne({ name }).lean<IPlan>();
    } catch (error) {
  ...
  async findAll(query: FilterQuery<Partial<IPlan>> = {}): Promise<IPlan[]> {
    try {
      ...
      return await this.model.find(filter).sort({ createdAt: -1 }).lean<IPlan[]>();
    } catch (error) {
  ...
  async findById(id: string): Promise<IPlan | null> {
    try {
      return await this.model.findById(id).lean<IPlan>();
    } catch (error) {
  ...
  async delete(id: string): Promise<IPlan | null> {
    try {
      return await this.model.findByIdAndDelete(id).lean<IPlan>();
    } catch (error) {
