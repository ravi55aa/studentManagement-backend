...
export default class PlanController {
    ...
    public async toggleActive(req: Request, res: Response, next: NextFunction): Promise<void> {
        ...
        if (typeof isActive !== 'boolean') {
            const { status, resBody } = ApiResponse.badRequest(SubscriptionPlanMessage.InvalidActiveStatus);
            res.status(status).json(resBody);
            return;
        }
    ...
    public async togglePopular(req: Request, res: Response, next: NextFunction): Promise<void> {
        ...
        if (typeof isPopular !== 'boolean') {
            const { status, resBody } = ApiResponse.badRequest(SubscriptionPlanMessage.InvalidPopularStatus);
            res.status(status).json(resBody);
            return;
        }
