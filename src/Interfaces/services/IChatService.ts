...
export interface IChatRoomService {
  createDirectChat(user1: string, user2: string): Promise<serviceReturnType>;

  createBatchChat(batchId: string): Promise<serviceReturnType>;

  getUserChats(userId: string): Promise<serviceReturnType>;

  getChatById(chatRoomId: string): Promise<serviceReturnType>;

  updateLastMessage(chatRoomId: string, message: string): Promise<serviceReturnType>;

  addParticipant(chatRoomId: string, userId: string): Promise<serviceReturnType>;
}
