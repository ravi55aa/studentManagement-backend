...
export interface IChatRoomRepository extends BaseRepository<IChatRoom> {
  ...
  findDirectChat(user1: string, user2: string): Promise<IChatRoom | null>;

  getUserChats(userId: string): Promise<IChatRoom[]>;

  updateLastMessage(chatRoomId: string, message: string): Promise<IChatRoom | null>;

  addParticipant(chatRoomId: string, userId: string): Promise<IChatRoom | null>;

  removeParticipant(chatRoomId: string, userId: string): Promise<IChatRoom | null>;
}
