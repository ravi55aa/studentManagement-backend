...
export class HomeworkRepository extends BaseRepository<IHomework> implements IHomeworkRepository {
  ...
  async getAllHomework(paginationQuery: TPaginationQuery, query: FilterQuery<Partial<IHomework>>): Promise<TPaginationResult<IHomework>|null> {
    try {
      ...
      const skip=(page - 1) * limit;
        ...
        const [data,total] = await Promise.all([
          ...
          this.model
          .find({ ...query, isDelete: false }).skip(skip).limit(limit)
          .populate('subjectId')
          .lean<IHomework[] | []>(),
