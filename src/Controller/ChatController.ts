...
import { IChatRoomService, IMessageService } from '@Interfaces/services/IChatService';
...
export default class ChatController {
  ...
  async createDirectChat(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const { status, resBody } = await this._chatRoomService.createDirectChat(user1, user2);
  ...
  //  Create Direct Chat
  async createBatchChat(req: Request, res: Response, next: NextFunction) {
    try {
      const { batchId } = req.body;

      const { status, resBody } = await this._chatRoomService.createBatchChat(batchId);
  ...
  async getUserChats(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const { status, resBody } = await this._chatRoomService.getUserChats(userId!);
  ...
  async getMessages(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const { status, resBody } = await this._chatMessageService.getMessages(chatRoomId!, {
        id: sender.adminId,
        role: sender.role,
      });
