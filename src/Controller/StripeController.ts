...
export default class StripeController {
  ...
  async createPaymentIntent(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const clientSecret = await this._service.createPaymentIntent(amount, studentFeeId, userId);
  ...
  async callWebHook(req: Request, res: Response) {
    ...
    try {
      ...
      return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({ received: false });
    }
  }
  ...
  async getStudentFeeDetails(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const { status, resBody }: serviceReturnType = await this._service.getStudentFeeDetails(
        studentId!,
      );
